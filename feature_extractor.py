...
class FeatureType(object):
    ...
    def __init__(self, feature_major_type, feature_name):
        assert feature_major_type == 'tag' or feature_major_type == 'label' or feature_major_type == 'word', 'unsupported feature major type ' + str(feature_major_type)

        self.major_type = feature_major_type    # 'label'
        self.name = feature_name                # 'stack.child(-1).sibling(1).label', etc
...
and each inherited index locator looks up a certain sibling/HEAD/etc of this token
'''
...
'''
Arguments: args[0]: get n'th child (< 0 indicates leftmost, > 0 indicates rightmost)
'''
class ChildFeatureLocator(ParserIndexLocator):
    ...
    def updateArgs(self, focus):
        ...
        if (levels < 0):
            ...
            logger.debug('ChildFeatureLocator: leftmostChild: levels=%d, focus=%d->%d' % (levels, oldfocus, focus.val))
        else:
            focus.val = self.parser.rightmostChild(focus.val, levels)
            logger.debug('ChildFeatureLocator: rightmostChild: levels=%d, focus=%d->%d' % (levels, oldfocus, focus.val))

'''
Arguments: args[0]: get n'th sibling (< 0 indicates to left, > 0 indicates to right)
'''
class SiblingFeatureLocator(ParserIndexLocator):
    ...
    def updateArgs(self, focus):
        ...
        if (position < 0):
            ...
            logger.debug('SiblingFeatureLocator: leftSibling: position=%d, focus=%d->%d' % (position, oldfocus, focus.val))
        else:
            focus.val = self.parser.rightSibling(focus.val, position)
            logger.debug('SiblingFeatureLocator: rightSibling: position=%d, focus=%d->%d' % (position, oldfocus, focus.val))

'''
Decodes a feature separated between a dot value into a feature name and argument list
...
class FeatureVector(object):
    ...
    However in reality we usually concatenate features with each other at a batch-level, not at a token-level
    '''
...
class SparseFeatureExtractor(object):
    ...
    def extractOne(self, parser, featureString):
        ...
        # must reference at least one focus and at least one feature (tag/label/etc), therefore, at least two elements
        ...
        # decodedParts: [('stack', [1]), ('child', [-1]), ('sibling', [1]), ('word', [])]
        assert len(decodedParts) >= 2
        assert(decodedParts[0][0] == 'input' or decodedParts[0][0] == 'stack')

        # start setting focus and follow focus modifiers until real feature (tag/label/etc)
        focus = None
        ...
        for d in decodedParts:
            ...
            fargs = d[1]
            if fname == 'input':
                assert feature_index == None, 'can\'t update focus if feature is already set'
                ...
                assert feature_index == None, 'can\'t request feature when feature is already set; nested features not supported'
                ...
                if fname == 'label':
                    ...
                    if focus.val == -1:
                        feature_index = self.feature_maps[feature_major_type].valueToIndex('<ROOT>')
                        self.logger.debug('%s: %d (%s)' % (feature_name, feature_index, '<ROOT>'))
                    elif focus.val < -1 or focus.val >= parser.numTokens():
                        feature_index = self.feature_maps[feature_major_type].valueToIndex('<OUTSIDE>')
                        self.logger.debug('%s: %d (%s)' % (feature_name, feature_index, '<OUTSIDE>'))
                    else:
                        ...
                        if feature_index == -1:
                            ...
                            self.logger.debug('%s: %d (%s)' % (feature_name, feature_index, self.feature_maps[feature_major_type].indexToValue(parser.label(focus.val))))
                elif fname == 'word':
                    ...
                    if focus.val < 0 or focus.val >= parser.numTokens():
                        ...
                        try:
                            feature_index = self.feature_maps[feature_major_type].valueToIndex(parser.getToken(focus.val).FORM)
                            self.logger.debug('%s: %d (%s)' % (feature_name, feature_index, parser.getToken(focus.val).FORM))
                        except: # Out of Vocabulary
                            feature_index = self.feature_maps[feature_major_type].valueToIndex('<UNKNOWN>')
                            self.logger.debug('%s: %d (%s)' % (feature_name, feature_index, '<UNKNOWN>'))
                elif fname == 'tag':
                    feature_major_type = 'tag'
                    if focus.val == -1:
                        # tag is based on BasicParserSentenceFeatureFunction
                        # the exceptional case for -1 happens in parser_features.h
                        # BasicParserSentenceFeatureFunction::Compute
                        # the strange thing is that the 'word' feature takes on <OUTSIDE> instead of <ROOT>. ???? bug in SyntaxNet? maybe because 'token.word' is nested, whereas simple 'tag' is not?
                        feature_index = self.feature_maps[feature_major_type].valueToIndex('<ROOT>')
                        self.logger.debug('%s: %d (%s)' % (feature_name, feature_index, '<ROOT>'))
                    elif focus.val < -1 or focus.val >= parser.numTokens():
                        feature_index = self.feature_maps[feature_major_type].valueToIndex('<OUTSIDE>')
                        ...
                        try:
                            feature_index = self.feature_maps[feature_major_type].valueToIndex(parser.getToken(focus.val).XPOSTAG)
                            self.logger.debug('%s: %d (%s)' % (feature_name, feature_index, parser.getToken(focus.val).XPOSTAG))
                        except: # Out of Vocabulary
                            feature_index = self.feature_maps[feature_major_type].valueToIndex('<UNKNOWN>')
                            self.logger.debug('%s: %d (%s)' % (feature_name, feature_index, '<UNKNOWN>'))
                else:
