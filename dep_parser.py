...
from arc_standard_transition_system import ArcStandardTransitionState, ArcStandardTransitionSystem
from gold_parse_reader import GoldParseReader
from decoded_parse_reader import DecodedParseReader
from gold_label_oracle import GoldLabelOracle
...
parser = argparse.ArgumentParser(
    description='Train a Chen and Manning-style neural network dependency parser')
...
parser.add_argument('--train', action='store_true', default=False,
                    help='Training a new model or continue training of an old model')
...
if (args.train and args.evaluate) or ((not args.train) and (not args.evaluate)):
    print('Please specify either training or evaluation mode (--train/--evaluate)')
    sys.exit(1)
...
class Parser(object):
    ...
    def setupParser(self, mode='train'):
        ...
        assert len(featureStrings) == len(set(featureStrings)), 'duplicate feature string detected'
        ...
        # Get major type groups in sorted order by contructing null parser state and extracting features
        # and then concatenating the similar types
        fvec = SparseFeatureExtractor(featureStrings, self.featureMaps).extract(ParserState(ConllSentence(), self.featureMaps))
        ...
        self.logger.info('Detected major feature groups (in alphabetical order): ' + str(self.featureMajorTypeGroups))

        self.featureDomainSizes = []
        self.featureEmbeddings = []

        # For now, use all same embedding sizes
        self.featureEmbeddingSizes = [embeddingSizes[t] for t in self.featureMajorTypeGroups] 
        ...
        for i in range(len(featureTypeInstances)):
            ...
            self.featureNames[major_type_index].append(featureTypeInstances[i].name)
            self.BAG_OF_FEATURES_LEN += (self.featureEmbeddingSizes[major_type_index])
        ...
        for i in range(len(self.featureMajorTypeGroups)):
            ...
            self.logger.info('... domain size: %d' % (self.featureMaps[major_type].getDomainSize()))
            self.logger.info('... embedding size: %d' % (self.featureEmbeddingSizes[i]))
            #self.logger.info('... feature count: %d' % (len(self.featureNames[i])))
            self.logger.info('... features')
            for fname in self.featureNames[i]:
                self.logger.info('....... %s' % (fname))
            self.logger.info('... total group embedding size: %d' % (len(self.featureNames[i]) * self.featureEmbeddingSizes[i]))

            self.logger.info('... initializing random normal embeddings...')
            self.featureDomainSizes.append(self.featureMaps[major_type].getDomainSize())
            ...
            with tf.variable_scope("feature_%s" % major_type):
                self.featureEmbeddings.append(tf.get_variable("embeddings", \
                    ...
                    initializer=tf.random_normal_initializer(stddev=1.0 / self.featureEmbeddingSizes[i]**.5, seed=0)))

        # FIXME:! i think the problem is that i have to save the embeddings and all the maps
        # because they aren't tensorflow variables, perhaps
        # we'll get there!!
        ...
        self.logger.info('Total feature count: %d' % (len(featureTypeInstances)))
        self.logger.info('Total bag of features length per state: %d' % (self.BAG_OF_FEATURES_LEN))
        self.logger.info('Total features input size: %d' % (batchSize*self.BAG_OF_FEATURES_LEN))

        # we'll use parser state's version of the action encoding
        self.ACTION_COUNT = self.transitionSystem.numActions(self.featureMaps['label'].getDomainSize())
    ...
    def setupVariables(self, mode='train'):
        ...
        self.X = tf.placeholder(tf.float32, [None, self.BAG_OF_FEATURES_LEN], name="ph_X")

        # Next action to take for given input parser state
        # Concatenate possible actions with arc labels
        # If action is SHIFT, ARC portion MUST BE COMPLETELY IGNORED and NEVER PARSED
        # We do not use a separate NULL value for the arc portion, therefore even though it
        # is a one-hot encoding it will be set to all zeros when action is SHIFT
        # Output: (, 3+~50)
        self.Y = tf.placeholder(tf.float32, [None, self.ACTION_COUNT], name="ph_Y")
        ...
        for i in range(len(hiddenLayerSizes)):
            ...
            with tf.variable_scope("layer_%d" % i):
                self.weights.append(tf.get_variable("weights", [prevLayerSize, h], initializer=tf.random_normal_initializer(stddev=1e-4, seed=0)))
                self.biases.append(tf.get_variable("biases", [h], initializer=tf.constant_initializer(0.2)))
            prevLayerSize = h
        
        # Output layer
        with tf.variable_scope("layer_out"):
            self.weights.append(tf.get_variable("weights", [prevLayerSize, self.ACTION_COUNT], initializer=tf.random_normal_initializer(stddev=1e-4, seed=0)))
            self.biases.append(tf.get_variable("biases", [self.ACTION_COUNT], initializer=tf.constant_initializer(0.2)))
        ...
        def greedy_graph_builder(x, weights, biases):
            ...
            for i in range(len(hiddenLayerSizes)):
                this_layer = tf.add(tf.matmul(last_layer, weights[i]), biases[i])
                this_layer = tf.nn.relu(this_layer)
                #this_layer = tf.nn.dropout(this_layer, 0.5) # TEST: don't do drop-out
        ...
        # Construct model
        self.hidden_layers, self.out_layer = greedy_graph_builder(self.X, self.weights, self.biases)
    ...
    def startTraining(self):
        ...
        reader = GoldParseReader(self.trainingCorpus, batchSize, featureStrings, self.featureMaps)
        ...
        with tf.Session(config=config) as sess:
            ...
            #self.beamSearchExperiment(sess)
            #self.doEvaluationAndPrintOutput(sess)
            #sys.exit(1)

            print_freq = 10 # every 10 batches, reset avg and print
            save_freq = 20
            eval_freq = 200

            i = 0
            while(True):
                self.logger.debug('Training i=%d: nextFeatureBags()' % i)
                reader_output = reader.nextFeatureBags()
                if reader_output[0] == None:
                    self.logger.debug('Training i=%d: reader output is None' % i)
                    break

                features_major_types, features_output, gold_actions, epoch_num = reader_output

                embeddings = []

                #print(features_major_types)
                #print(features_output)
                #print(gold_actions)

                # order of features will match the order of our arrays above
                for k in range(len(features_output)):
                    embeddings.append(tf.nn.embedding_lookup(self.featureEmbeddings[k], features_output[k]))
                    embeddings[-1] = tf.reshape(embeddings[-1], [-1, len(self.featureNames[k]) * self.featureEmbeddingSizes[k]])
                    
                #print('Y', gold_actions)

                tf_batch_x = tf.concat(1, embeddings)
                #print('X shape: ', self.X.get_shape())

                tf_batch_y = tf.one_hot(indices=tf.constant(gold_actions), depth=self.ACTION_COUNT, on_value=1.0, off_value=0.0, axis=-1)

                _, c = sess.run([self.optimizer, self.cost], feed_dict={self.X: tf_batch_x.eval(),
                                                                        self.Y: tf_batch_y.eval()})
                avg_cost += c / len(gold_actions) # to divide by number of actual batch items returned

                #if epoch_num != epoch_num_old:
                #    print("Epoch:", '%04d' % (epoch_num), "cost=", \
                #        "{:.9e}".format(avg_cost))
                #    avg_cost = 0.0

                if i % print_freq == 0:
                    self.logger.info('Epoch: %04d cost=%s' % (epoch_num, \
                        "{:.9e}".format(avg_cost)))
    ...
    def doEvaluation(self, sess):
        ...
        # Start getting sentence batches...
        test_reader = GoldParseReader(self.testingCorpus, batchSize, featureStrings, self.featureMaps, epoch_print=False)
        ...
        for i in range(test_runs):
            ...
            features_major_types, features_output, gold_actions, epoch_num = test_reader_output
            ...
            Y_actual = tf.one_hot(indices=tf.constant(gold_actions), depth=self.ACTION_COUNT, on_value=1.0, off_value=0.0, axis=-1).eval()
            ...
            for i in range(len(Y_actual)):
                ...
                #print('pred', predActionCombined, 'actual', actualActionCombined)

                predAction = self.transitionSystem.actionType(predActionCombined)
                actualAction = self.transitionSystem.actionType(actualActionCombined)
                ...
                if actualAction == ParserState.SHIFT:
                    ...
                    actualTag = self.transitionSystem.label(actualActionCombined)
        ...
        # errors that don't accumulate (tokens are tested individually with gold inputs)
        self.logger.info('non-accumulated error measure')
        self.logger.info('test_set (LAS): %d/%d (%.2f%%), test_set (UAS): %d/%d (%.2f%%)' % (correctElems, totalElems, 100.0 * (float(correctElems) / float(totalElems)), correctActions, totalElems, 100.0 * (float(correctActions) / float(totalElems))))
    ...
    def beamSearchExperiment(self, sess):
        ...
        # of course, token errors can accumulate and this is why sentence-wide accuracy is lower
        # than token-only accuracy given by doEvaluation()

        # batch size set at one temporarily to get one sentence
        test_reader_decoded = DecodedParseReader(self.testingCorpus, 1, featureStrings, self.featureMaps, epoch_print=False)
        ...
        for i in range(test_runs):
            print('test_run', i)
            test_reader_output = test_reader_decoded.nextFeatureBags(pred_top_k_var, filled_count)
            if test_reader_output[0] == None:
                print('eval error')
                return
            
            features_major_types, features_output, epochs, filled_count = test_reader_output

            print('len(features_output[0]):', len(features_output[0]))
            print('len(features_output[1]):', len(features_output[1]))
            print('len(features_output[2]):', len(features_output[2]))
            ...
            Y_pred_top_k = sess.run(self.pred_top_k, feed_dict={self.X: tf_batch_x.eval()})
            ...
            #Y_pred = sess.run(pred, feed_dict={X: tf_batch_x.eval()})

            #print(Y_pred_top_k[0][0])
            #print(Y_pred_top_k[0][1])
            #print(Y_pred_top_k[1][0])
            #print(Y_pred_top_k[1][1])
            #print(Y_pred[0])

            #pred_top_k_var = []
            #for k in range(len(Y_pred)):
            #    pred_top_k_var.append([np.argmax(Y_pred[k])])

            #print('len(Y_actual)', len(Y_actual))
            #print('len(Y_pred_top_k)', len(Y_pred_top_k))
            #print('len(Y_pred_top_k[0])', len(Y_pred_top_k[0]))
            ...
            #print(predActionCombined, actualActionCombined)

            #predAction = GlobalTransitionSystem.actionType(predActionCombined)
            #actualAction = GlobalTransitionSystem.actionType(actualActionCombined)

            #if predAction == ParserState.SHIFT:
            #    predTag = None
            #else:
            #    predTag = GlobalTransitionSystem.label(predActionCombined)

            #if actualAction == ParserState.SHIFT:
            #    actualTag = None
            #else:
            #    actualTag = GlobalTransitionSystem.label(actualActionCombined)
        ...
        # also print out number of correct actions (even if tag was wrong)
        #logger.info('Correct actions+arcs: %d/%d, correct actions: %d' % (correctElems, totalElems, correctActions))

        # we need to make sure unseen words get mapped to unknown values when we use an unseen corpus
        ...
        for sentences in outputs:
            ...
            for sentence in sentences:
                ...
                for w in sentence.tokens:
                    ...
                    #print(w.FORM)
                    #print(w.parsedLabel, "<-->", w.DEPREL)
                    #print(w.parsedHead, "<-->", w.HEAD)
                    ...
                    if w.parsedLabel == gold_deprel and w.parsedHead == gold_head:
                    ...
                    if w.parsedHead == -1:
                        print('%-20s%-10s%-5d%-5s' % (w.FORM, 'ROOT', w.parsedHead, suffix))
                    else:
                        print('%-20s%-10s%-5d%-5s' % (w.FORM, w.parsedLabel, w.parsedHead, suffix))

        # errors that accumulate (tokens are tested based on previous decoded decisions, which could screw up shifting and arcing, etc)
        logger.info('accumulated error measure')
        logger.info('accuracy(UAS): %d/%d' % (head_correct, token_count))
        logger.info('accuracy(LAS): %d/%d' % (deprel_and_head_correct, token_count))
        logger.info('accuracy(deprel correct): %d/%d' % (deprel_correct, token_count))
    ...
    def doEvaluationAndPrintOutput(self, sess):
        ...
        test_reader_decoded = DecodedParseReader(self.testingCorpus, batchSize, featureStrings, self.featureMaps, epoch_print=False)
...
configFile = open(modelParams.getFilePath('parser-config'), 'r', encoding='utf-8').read()
