...
class Parser(object):
    ...
    def buildNetwork(self, mode='train'):
        ...
        with tf.name_scope(mode):
            ...
            # problem is, training RNN parameters were getting trained
            # but the eval ones didn't get the set values
            # need to fix all of this
            with tf.variable_scope('RNNboth', reuse=(mode=='eval')):
                # Reshape the tensor back to original shape so that it can be fed
                # into RNN layers
                last_layer = tf.stack(tf.split(axis=0, num_or_size_splits=batchSize,
                    value=last_layer), axis=0)
                cell = tf.contrib.rnn.LSTMCell(last_layer_size, state_is_tuple=True)
                #self.inits.append(cell._initializer)
                val, _ = tf.nn.dynamic_rnn(cell, last_layer, dtype=tf.float32)
                print('val.get_shape()', val.get_shape())
                val = tf.reshape(val, [-1, last_layer_size])
                print('val.get_shape()', val.get_shape())
                #val = tf.transpose(val, [1, 0, 2])
                #val = tf.transpose(val, [0, 2, 1])
                #print('val.get_shape()', val.get_shape())

                #last_layer = tf.gather(val, int(val.get_shape()[0]) - 1)
                last_layer = val
                print('last_layer.get_shape()', last_layer.get_shape())
                # last_layer = val
            ...
            if mode == 'train':
                ...
                #lstm_params = [v for v in tf.all_variables() if
                #    v.name.startswith('train/rnn/')]

                lstm_params = [v for v in tf.all_variables() if
                    v.name.startswith('RNNboth/')]
                ...
                regularized_params += [tf.nn.l2_loss(p) for p in lstm_params]
                ...
                trainableParams = list(self.params.values())

                ## FIXME
                trainableParams += lstm_params

                print('trainableParams', [v.name for v in trainableParams])
    ...
    def attachmentMetric(self, sess, runs=200, mode='testing'):
        ...
        for i in range(test_runs):
            ...
            ## RNN FIXME ##
            ## InvalidArgumentError (see above for traceback): Number of ways to split should evenly divide the split dimension, but got split_dim 0 (size = 486) and num_split 512
            if filled_count < batchSize:
                break
...
def __main__():
    ...
    if args.train:
        ...
        with tf.Session(config=config) as sess:
            ...
            parser.buildNetwork('eval')
            # init after all have been built
